...
public class SchedulerArgsTest {
    ...
    public void test_cron_serialize() throws JSONException {
        ...
        final SchedulerArgs req = SchedulerArgs.builder().job(j1).trigger(t1).build();
    ...
    public void test_cron_deserialize() {
        final QWEJobModel j1 = JobModelCreator.create("abc");
        final TriggerModel t1 = CronTriggerModel.builder().name("t1").expr("0 0/1 * 1/1 * ? *").build();
        final SchedulerArgs data = JsonData.from("{\"job\":{\"type\":\"EVENTBUS_JOB\",\"name\":\"abc\"," +
        ...
        Assert.assertEquals(SchedulerArgs.builder().job(j1).trigger(t1).build(), data);
    }
