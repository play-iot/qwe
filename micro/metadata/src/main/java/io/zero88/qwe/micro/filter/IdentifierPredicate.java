...
final class IdentifierPredicate implements RecordPredicate {
    ...
    public @NonNull Predicate<Record> apply(@NonNull JsonObject filter) {
        String identifier = filter.getString(ServiceLocatorParams.IDENTIFIER);
        if (Strings.isBlank(identifier)) {
            return r -> true;
        }
        return ByPredicate.ByPredicateEnum.parse(action, filter.getString(ServiceLocatorParams.BY)).by(identifier);
    }
