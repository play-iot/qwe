...
import io.zero88.qwe.event.mock.MockReceiveSendOrPublishListener;
import io.zero88.qwe.event.mock.MockRx2Listener;
...
public class EventListenerTest {
    ...
    void test_request_error(VertxTestContext testContext) {
        ...
        eventBusClient.register(address, new MockListenerFailed());
    ...
    void test_publish(VertxTestContext testContext) {
        ...
        eventBusClient.register(address, new MockReceiveSendOrPublishListener("id1", checkpoint));
        eventBusClient.register(address, new MockReceiveSendOrPublishListener("id2", checkpoint));
    ...
    void test_rx2_single(VertxTestContext testContext) {
        ...
        eventBusClient.request(address, EventMessage.initial(action, new JsonObject().put("id", 111)))
                      .onSuccess(msg -> testContext.verify(() -> {
                          System.out.println(msg.toJson());
                          Assertions.assertEquals(EventAction.REPLY, msg.getAction());
